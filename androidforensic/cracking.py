...
class PasswordCrack:
    ...
    def set_prog(self, obj, n, total):
        if obj:
            ...
            obj.set(f'{done_:,.2f} % \t{rem_} remaining')

    def crack_password(self, callback=None, stop_check=None, tried=None, rate=None, prog=None):
        """Crack password with interface-agnostic callbacks.

        Args:
            callback: Callable that receives current pin attempt string. None = no UI updates.
            stop_check: Callable returning bool. True = stop cracking. None = never stop.
            tried: Object with .set(str) method for tried count. None = skip.
            rate: Object with .set(str) method for rate display. None = skip.
            prog: Object with .set(str) method for progress display. None = skip.
        """
        ...
        for n, pin in enumerate(feed, start=1):
            if not n % self.update_rate and callback:
