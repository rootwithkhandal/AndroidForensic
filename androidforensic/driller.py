...
class ChainExecution:
    ...
    def update(self, msg, info=True):
        """Interface-agnostic status update.

        Works with:
        - Callable: called with msg string (CLI/TUI/Web callback)
        - Object with .set() method: called like Tkinter StringVar (legacy compat)
        - None: only logs, no UI update
        """
        self.logger.info(msg) if info else logger.debug(msg)
        if self.updater:
            if callable(self.updater):
                self.updater(msg)
            elif hasattr(self.updater, 'set'):
    ...
    def CreateWorkDir(self):
        ...
        try:
            ...
            self.work_dir = os.path.join(self.base_dir, f'androidforensic_extraction_{date_}_{time_}')
    ...
    def do_backup(self, ALL=True, shared=False, backup_name='backup.ab'):
        ...
        com.start()
        if self.updater:
            # For web/tui, the caller handles prompts
            self.update(messages.backup_instructions())
        while com.is_alive():
            time.sleep(0.5)
