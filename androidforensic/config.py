...
# Auto-copy bundled bin directory from legacy if it doesn't exist
try:
    _current_dir = pathlib.Path(__file__).absolute().parent
    _bin_dir = _current_dir / 'bin'
    _legacy_dir = _current_dir.parent / 'andriller' / 'bin'
    if not _bin_dir.exists() and _legacy_dir.exists():
        import shutil
        shutil.copytree(_legacy_dir, _bin_dir)
except Exception:
    pass
