...
from androidforensic import adb_conn
...
@pytest.fixture
def ADB(mocker):
    mocker.patch('androidforensic.adb_conn.ADBConn.kill')
    mocker.patch('androidforensic.adb_conn.ADBConn._opt_use_capture', return_value=True)
    with mock.patch('androidforensic.adb_conn.ADBConn._get_adb_bin', return_value=fake_adb.name):
        with mock.patch('androidforensic.adb_conn.ADBConn._adb_has_exec', return_value=True):
            adb = adb_conn.ADBConn()
    adb.adb_bin = fake_adb.name
...
def ADB_alt(mocker):
    mocker.patch('androidforensic.adb_conn.ADBConn.kill')
    mocker.patch('androidforensic.adb_conn.ADBConn._opt_use_capture', return_value=False)
    with mock.patch('androidforensic.adb_conn.ADBConn._get_adb_bin', return_value=fake_adb.name):
        with mock.patch('androidforensic.adb_conn.ADBConn._adb_has_exec', return_value=False):
            adb = adb_conn.ADBConn()
    adb.adb_bin = fake_adb.name
    adb_cmd = adb.adb.__func__
    setattr(adb, 'adb', lambda *args, **kwargs: adb_cmd(adb, *args, **kwargs))
...
@pytest.fixture
def ADB_win(mocker):
    mock_sub = mocker.patch('androidforensic.adb_conn.subprocess', autospec=True)
...
def test_adb_simple(ADB, mocker):
    ...
    mock_run = mocker.patch('androidforensic.adb_conn.subprocess.run', return_value=output)

    res = ADB('hello')
    assert res == 'lala'
    mock_run.assert_called_with([fake_adb.name, 'hello'],
        capture_output=True, shell=False, startupinfo=ADB.startupinfo)
...
def test_adb_out_alt(ADB_alt, mocker):
    ...
    mock_run.assert_called_with([fake_adb.name, 'shell', 'id'],
        stdout=subprocess.PIPE, shell=False, startupinfo=ADB_alt.startupinfo)
