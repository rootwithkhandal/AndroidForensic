...
from androidforensic import __version__


@pytest.fixture
def conf(mocker):
    tmpdir = tempfile.mkdtemp()
    mocker.patch('androidforensic.config.AppDirs.user_config_dir', new_callable=mock.PropertyMock, return_value=tmpdir)
    yield config.Config()
...
def test_check_latest_version(conf, version, result):
    ...
    with mock.patch('androidforensic.config.requests.get', return_value=response_obj):
