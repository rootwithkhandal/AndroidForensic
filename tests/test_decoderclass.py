...
import hashlib
import tempfile
from androidforensic.classes import AndroidDecoder, DecoderError
...
def test_unix_to_time(Deco):
    import datetime
    Deco.conf.tzone = datetime.timezone(datetime.timedelta(hours=0))
    Deco.conf.date_format = '%Y-%m-%d %H:%M:%S %Z'
